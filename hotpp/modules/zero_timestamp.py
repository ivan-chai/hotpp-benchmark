...
class ZeroTimestampsEncoder(torch.nn.Module):
    def __init__(self, num_classes, delta=0, timestamps_field="timestamps", labels_field="labels"):
        super().__init__()
    ...
    def forward(self, x, return_full_states=False):
        ...
        with deterministic(False):
            deltas = torch.full_like(deltas, self._delta)
...
class ZeroTimestampsModule(BaseModule):
    ...
    """
    def __init__(self, k, num_classes, delta=0,
                 timestamps_field="timestamps",
                 labels_field="labels",
                 val_metric=None,
                 test_metric=None,
                 **kwargs):
        super().__init__(seq_encoder=ZeroTimestampsEncoder(num_classes, delta, timestamps_field=timestamps_field, labels_field=labels_field),
                         loss=Identity(2),
