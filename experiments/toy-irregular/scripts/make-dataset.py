...
def parse_args():
    ...
    parser.add_argument("--max-length", help="Dataset size", type=int, default=100)
    parser.add_argument("--large-step", help="The fraction of large time steps", type=float, default=20)
    parser.add_argument("--large-value", help="Large time step", type=float, default=1.0)
    parser.add_argument("--small-value", help="Small time step", type=float, default=0.0)
...
def main(args):
    ...
    np.random.seed(0)
    labels = np.random.choice(1, p=[1], size=(b, l))
    timedeltas = np.full((b, l), args.small_value)
    timedeltas[:, ::args.large_step] = args.large_value
    timestamps = np.cumsum(timedeltas, 1)
