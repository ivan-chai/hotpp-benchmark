...
TRANSFORMERS = {
    "AttNHP": AttNHPTransformer
}

KWARGS = {
    "AttNHP": [{}, {"dim_feedforward": 0}, {"dim_value": HIDDEN_SIZE // 2}]
}

# Optional dependency.
try:
    from hotpp.nn.encoder import ContiformerTransformer
    TRANSFORMERS["Contiformer"] = ContiformerTransformer
    KWARGS["Contiformer"] = [{}, {"dim_feedforward": 0}, {"dim_kv": HIDDEN_SIZE // 2}]
except ImportError:
    print("Skip Contiformer tests (not installed).")
...
class TestTransformer(TestCase):
    def test_output_is_equal_to_decode(self):
        ...
        n_heads = 4
        n_layers = 3
        for name, model_cls in TRANSFORMERS.items():
            for kwargs in KWARGS[name]:
                print(f"Test {name} transformer with {kwargs}")
                model = model_cls(input_size, hidden_size, n_heads, n_layers, **kwargs).eval()
